...
def compute_th(PSD, barks, ATH, freqs,freq_stft):
    ...
    masking[:1025]=10 * np.log10(np.sum(pow(10, Ts / 10.),axis=0))
    return theta_x
...
def generate_th(audio, fs, window_size=2048):
    ...
    freq_stft=librosa.core.stft(audio, center=False)
...
for i in range(1):
    sample_rate_np, audio_temp = wav.read('original1.wav')
    if max(audio_temp) < 1:
